...
export default function Slide34({ updateFormData, formData }: Props) {
  // Smart persistence: use existing formData value or default
  const existingSelection = formData?.mannenShampooIngredient || "Vitamine B5"
  ...
  useEffect(() => {
    ...
    if (!formData?.mannenShampooIngredient) {
      const updates = {
        mannenShampooIngredient: "Vitamine B5",
        marketingIngredientenMannenShampoo: "Vitamine B5"
      }
      ...
      try {
        localStorage.setItem('salonid:Mannen ShampooIngredient', "Vitamine B5")
  ...
  const handleOptionSelect = (choice: string) => {
    ...
    updateFormData({ 
      mannenShampooIngredient: choice,
      // Also update the marketing ingredient field for database mapping
      marketingIngredientenMannenShampoo: choice
    })

    // Persist to localStorage
    try {
      localStorage.setItem('salonid:Mannen ShampooIngredient', choice)
      localStorage.setItem("salonid:dateISO", new Date().toISOString())
    } catch (error) {
      // localStorage not available, continue silently
    }
    ...
    <SlideContainer width="wide">
      <section>
        <h2 className={getTypographyClasses("title", { alignment: "left" })}>
          Mannen - Kies de ingrediënten
        </h2>

        <div className="max-w-[760px] space-y-4 mb-8">
          <p className={getTypographyClasses("paragraph", { alignment: "left" })}>
            Selecteer het gewenste ingrediënt voor jouw Mannen Shampoo. Elk ingrediënt biedt unieke voordelen voor de haarverzorging.
          </p>
        </div>

        {/* Single Product Section: Mannen Shampoo */}
        <div className="space-y-8">
          <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
            <h3 className="text-xl font-semibold text-gray-900 mb-6">Mannen Shampoo</h3>
